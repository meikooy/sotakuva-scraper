...
function getImagesFromIndex(from) {
    ...
    request(url, function(error, response, html) {
        if (!error) {
            ...
            $('.lightdesk_image').each(function() {
                ...
                // Asyncronously update details for the image
                detailRequests.push(updateDefailsForImage(image));

                images.push(image);
            });
            
            Promise.all(detailRequests).then(images => {
                photosIndex.addObjects(images, (err, content) => {
                    if (err) {
                        console.log(err);
                    }
                });
            ...
            if (images.length > 0) getImagesFromIndex(total);
            else {
                console.log(html);
            }
        }
